      ...
      {characteristics.map((c) => (
        ...
        >
          <div className="flex items-center">
            <span className="text-white/50 text-lg whitespace-nowrap mr-2">{c.label}</span>
            <span className="border-b border-dashed border-white/50 flex-1 mt-2.75"></span>
          </div>
          <span className="text-white text-lg text-left whitespace-nowrap ml-2">{c.value}</span>
        </div>
      ))}
    </div>
  );
